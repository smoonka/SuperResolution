function [hIm] = sparse_coding(lIm, zooming, patch_size, overlap, Dh, Dl,regres,n)
...
%bhIm = mIm;
hIm = zeros([hhg, hwd]);
%bhIm = zeros(size(mIm,1),size(mIm,2));
        ...
        %if mnorm > 1,
         %     hpatch = Dh*w*mnorm;
          %   else
           %   hpatch = Dh*w;
        %end
        %w = SolveLasso(Dl, y, size(Dl, 2), 'nnlasso', [], .1);
		 % w = doProject(y, Dl',n);
         w = feature_sign(Dl, y, .1);
